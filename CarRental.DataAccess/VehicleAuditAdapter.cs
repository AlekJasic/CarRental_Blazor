...
{
    /// <summary>
    /// This class is a helper class to audit <see cref="Vehicle"/> instances.
    /// </summary>
    public class VehicleAuditAdapter
    {
        ...
        /// <param name="context">The <see cref="VehicleContext"/> to use.</param>
        /// <param name="saveChangesAsync">A delegate to save the changes.</param>
        /// <returns>A <see cref="Task"/>.</returns>
        public async Task<int> ProcessVehicleChangesAsync(
            ...
            Func<Task<int>> saveChangesAsync)
        {
            ...
            var audits = new List<VehicleAudit>();

            // audit vehicles
            foreach (var item in context.ChangeTracker.Entries<Vehicle>())
            {
                    ...
                    item.State == EntityState.Deleted)
                {
                    ...
                    {
                        item.Property<string>(VehicleContext.CreatedBy).CurrentValue =
                            user;
                        item.Property<DateTimeOffset>(VehicleContext.CreatedOn).CurrentValue =
                    ...
                    if (item.State == EntityState.Modified)
                    {
                        ...
                        dbVal = db.ToObject() as Vehicle;
                        item.Property<string>(VehicleContext.ModifiedBy).CurrentValue =
                            user;
                        item.Property<DateTimeOffset>(VehicleContext.ModifiedOn).CurrentValue =
                          DateTimeOffset.UtcNow;
                    }

                    // parse the changes
                    var changes = new PropertyChanges<Vehicle>(item, dbVal);
                    var audit = new VehicleAudit
                    {
                        VehicleId = item.Entity.Id,
                        Action = item.State.ToString(),
                        User = user,
                        Changes = JsonSerializer.Serialize(changes),
                        VehicleRef = item.Entity
                    };
            ...
            {
                // save
                context.VehicleAudits.AddRange(audits);
            }
            ...
            foreach (var audit in audits.Where(a => a.VehicleId == 0).ToList())
            {
                secondSave = true;
                audit.VehicleId = audit.VehicleRef.Id;
